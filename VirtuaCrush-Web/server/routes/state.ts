...
// the logged-in user: mood, emotion gauges, secret progress, goal.
...
// GET /api/state/:characterId — current mood/emotions/secret/goal
router.get('/:characterId', requireAuth, async (req: Request, res: Response) => {
  const { characterId } = req.params;
  try {
    const { state } = await getSituation(req.user!.id, characterId);
