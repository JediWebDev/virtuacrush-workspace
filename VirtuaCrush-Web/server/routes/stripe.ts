...
const stripe = new Stripe(process.env.STRIPE_SECRET_KEY!, {
  // Pin to a tested API version. Update intentionally, not by accident.
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        // Fetch the subscription to get period_end + status in one shot.
      ...
      case 'customer.subscription.deleted': {
        ...
        if (!userId) break;   // Unknown customer; ignore.
