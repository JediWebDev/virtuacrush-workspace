...
export async function generateChoice(params: {
  ...
  try {
    const result = await completePrompt(buildChoicePrompt(params));
...
// Thin shim so the date/bill call sites below can keep their
// `await llm.generateContentComplete({ prompt })` shape while routing through
// the provider layer.
function llmComplete() {
  return Promise.resolve({
    generateContentComplete: ({ prompt }: { prompt: string }) => completePrompt(prompt),
  });
}
