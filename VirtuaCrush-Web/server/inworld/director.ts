...
PACING: ${stage.arcContext.isMeetArc
  ...
  const skillCheckRules = resolutionMode
    ? ''
    : `
DUNGEON MASTER — SKILL CHECKS: Judge the player's last action. If it is clearly impossible, superhuman, or reality-breaking (flying, super strength, teleporting, summoning, destroying the world), you MUST set "skillCheck" at "formidable" — do NOT let ${stage.companionName} or the narrator simply describe it working. Also set a check for actions that are genuinely uncertain, risky, or yank the story in a wild new direction (pick the fitting difficulty). On these turns ${stage.companionName}/narrator only REACT and set the moment up; nobody confirms the feat succeeded — leave the outcome hanging for the dice. Fill "action" (their attempt) and "difficulty" (trivial=5, easy=8, medium=12, hard=16, formidable=19). Write "prompt": the Dungeon Master's own voice, second person, punchy — react to the attempt and call for the roll, naming the exact target number, e.g. "So you can suddenly fly? Bold. Give me a d20 — beat a 16 or gravity wins." IMPORTANT — if ${stage.companionName} is remote / not in the same place as the player, they CANNOT see the attempt: have them react only to what they could actually perceive (a sound, the line going quiet, the world itself changing) — never describe the player's body or moves as if watching them. For ordinary, safe, emotional, or purely conversational actions, OMIT skillCheck (or set required:false). Never demand a roll for normal dialogue or feelings.`;
